            ...
            <div class="layui-form-item">
                <div class="layui-inline">
                    ...
                    <div class="layui-input-inline">
                        <input type="text" name="unitCode" autocomplete="off" class="layui-input">
                    </div>
                </div>
                <div class="layui-inline">
    ...
    layui.use(['form', 'table', 'spLayer', 'spTable'], function () {
        ...
        table.on('toolbar(js-record-table-filter)', function (obj) {
            ...
            if (obj.event === 'add') {
                var index = spLayer.open({
                    title: '添加加工单元',
                    area: ['90%', '90%'],
                    content: '${request.contextPath}/basedata/workUnit/add-or-update-ui'
                });
            }
        });
        ...
        table.on('tool(js-record-table-filter)', function (obj) {
            ...
            if (obj.event === 'delete') {
                layer.confirm('确认要删除吗？', function (index) {
                    spUtil.ajax({
                        url: '${request.contextPath}/basedata/workUnit/delete',
                        async: false,
                        type: 'POST',
                        showLoading: true,
                        serializable: false,
